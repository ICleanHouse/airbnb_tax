...
- **Cleaner action from map**: authenticated cleaner users see an `Offer cleaning` popup button that submits a normal cleaner application through `POST /api/marketplace/applications/`. Guests and hosts do not see the action.
